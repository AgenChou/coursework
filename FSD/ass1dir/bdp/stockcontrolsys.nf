...
THEORY ListSubstitutionX IS
  Expanded_List_Substitution(Machine(stockcontrolsys),complicatedserviceorder)==(size(orders)>0 | @oos.(oos = {oo | oo: ran(orders) & !ii.(ii: dom(oo) => oo(ii)<=stock(ii))} ==> @nn.(nn = min(dom(orders|>oos)) ==> stock,orders:=stock<+%ii.(ii: dom(orders(nn)) | stock(ii)-orders(nn)(ii)),orders/|\nn-1^(orders\|/nn))));
  ...
  Expanded_List_Substitution(Machine(stockcontrolsys),receivestock)==(ss: ITEMS +-> NAT1 | stock:=stock<+ss);
  ...
  List_Substitution(Machine(stockcontrolsys),receivestock)==(stock:=stock<+ss);
  ...
  List_Substitution(Machine(stockcontrolsys),complicatedserviceorder)==(LET oos BE oos = {oo | oo: ran(orders) & !ii.(ii: dom(oo) => oo(ii)<=stock(ii))} IN LET nn BE nn = min(dom(orders|>oos)) IN stock:=stock<+%ii.(ii: dom(orders(nn)) | stock(ii)-orders(nn)(ii)) || orders:=orders/|\nn-1^(orders\|/nn) END END)
END
&
