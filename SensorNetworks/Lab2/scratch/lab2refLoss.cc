...
int main (int argc, char *argv[])
{
  ...
  txp = -10.0; // Initial power value. Keep it constant
  refLoss = 5; // 

  for (int p = 0; p < 31; p++) //Do this for 20 different power values starting at -10 dB with a 0.5 dB increment in each iteration
  {

        distance = 10;

	for (int d = 0; d < 40; d++) //Do this for 40 different distance values starting at 10 m with a 5 m increment in each iteration
        {
		...
		//Create custom traffic source and destination(sink)
  		...
  		out.close ();
                ...
                distance = distance + 5; //Increase the distance by 0.1 m for next iteration
        ...
        refLoss = refLoss + 0.5;  //Increase the power by 0.005 dB for next iteration
