	...
	for (int d = 0; d < 10; d++) //Do this for 10 different distance values starting at 180 m with a 0.10 m increment in each iteration
        {
  		...
  		YansWifiChannelHelper wifiChannel;
                ...
                std::string lossModel = "ns3::FriisPropagationLossModel";//define the name of the propagation loss model
                ...
                //std::string atr2 = "SystemLoss";// The distance at which the reference loss is calculated (m)
                //std::string atr3 = "MinDistance";// The reference loss at reference distance (dB)

  		//Here, we add the propagation loss model to the channel, with all the required parameters
  		wifiChannel.AddPropagationLoss (lossModel, atr1, DoubleValue(3000000000));
