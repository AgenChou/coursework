...
int main (int argc, char *argv[])
{
  ...
  txp = -10; //Initial power value

  for (int p = 0; p < 40; p++) //Do this for 5 different power values starting at 0.1 dB with a 0.005 dB increment in each iteration
  {

        distance = 10;

	for (int d = 0; d < 30; d++) //Do this for 10 different distance values starting at 180 m with a 0.10 m increment in each iteration
        {
  		...
  		wifiChannel.AddPropagationLoss (lossModel, atr1, DoubleValue(exponent), atr2, DoubleValue(1), atr3, DoubleValue(19.41));
		...
		InternetStackHelper internet;
  		...
  		NS_LOG_UNCOND ("Distance: " << distance << " exponent: " << exponent);
		...
		//Create custom traffic source and destination(sink)
  		...
  		out.close ();
                ...
                distance = distance + 10; //Increase the distance by 0.1 m for next iteration
        ...
        exponent += 0.25;  //Increase the power by 0.005 dB for next iteration
